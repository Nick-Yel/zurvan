...
export default function RouteThreePage() {
  ...
  const handleFileChange = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      setIsUploading(true);
      const response = await fetch("/api/upload/kindness", {
        method: "POST",
        body: data,
      });
  ...
  return (
    <main className="w-full relative">
      <div className="absolute inset-0 bg-cover bg-top bg-no-repeat bg-fixed transition-opacity duration-500" style={{ backgroundImage: bgLoaded ? 'url(/assets/page-3.jpg)' : 'none', opacity: bgLoaded ? 0.5 : 0 }} />
